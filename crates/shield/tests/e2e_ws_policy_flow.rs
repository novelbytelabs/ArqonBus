...
#[tokio::test]
async fn e2e_ws_policy_allows_non_empty_and_blocks_empty_binary() {
    let (url, handle, recorder) = spawn_server().await;
    let token = issue_token("tenant-e2e");
    ...
    ws.send(Message::Binary(vec![9_u8, 8, 7].into()))
        .await
        .expect("send non-empty");
    let first = ws.next().await.expect("echo").expect("echo ok");
    assert!(matches!(first, Message::Binary(_)));

    // Empty payload should be denied by policy and not echoed.
    ws.send(Message::Binary(Vec::new().into()))
        .await
        .expect("send empty");
    let blocked = timeout(Duration::from_millis(150), ws.next()).await;
    assert!(
        blocked.is_err(),
        "empty payload should be blocked with no echo"
    );
    ...
    assert_eq!(
        records.len(),
        1,
        "only non-empty payload should be published to NATS"
    );
    assert_eq!(records[0].subject, "in.t.tenant-e2e.raw");
